...
[proFit] = pro_fitAll(choice, outcome, testFlag); sprintf('Pro finished')
...
% Predicted response outcome model
proModels = fields(proFit);
proBIC = NaN(1, length(proModels));
proLH = NaN(1, length(proModels));
for i = 1:length(proModels)
    proBIC(i) = proFit.(proModels{i}).BIC;
    proLH(i) = proFit.(proModels{i}).LH;
end
[~, best_proModel] = min(proBIC);
probChoice_pro = proFit.(proModels{best_proModel}).probChoice;
...
tMax = length(behSessionData);

% Raw data plot
subplot(fig_rawData); hold on
plot_rawData_operantMatching(fig_rawData, behSessionData, unCorrectedBlockSwitch, tMax);
...
probChosenChoice_pro = zeros(length(probChoice_pro), 1);
for i = 1:length(probChoice_qLearning)
    ...
    tmp = probChoice_pro(i, :);
    probChosenChoice_pro(i) = tmp(logical(choice(i, :)));
end
...
plot(xvals, probChosenChoice_pro, 'Color', pro_color, 'linewidth', 1)
...
plot(xvals, probChoice_pro(:, 1)*2 - 1, 'Color', pro_color, 'linewidth', 1)
...
allBIC = [qBIC daBIC davBIC acfBIC dtsBIC spBIC proBIC];
...
all_bestInd = [best_qLearningModel best_directActorModel best_directActorWithValueModel best_acfModel best_dynamicThompsonModel best_stackedProbModel best_proModel];
modelLen = [length(qBIC) length(daBIC) length(davBIC) length(acfBIC) length(dtsBIC) length(spBIC) length(proBIC)];
...
if saveFigFlag == 1
    ...
    saveFigurePDF(gcf,[savepath sep out.saveFigName '_rlFits'])
end