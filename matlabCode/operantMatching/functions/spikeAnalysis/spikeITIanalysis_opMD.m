function [mdlStruct] = spikeITIanalysis_opMD(sessionName, varargin)
...
for i = 1:length(cellInd)
    ...
    plotShaded([-p.Results.tb*1000 500], [0 0; 0 100], 'k') 
    ...
    ylim(z(7),[SDFyLimMin SDFyLimMax])
    ...
    if p.Results.saveFigFlag == 1 
        saveFigurePDF(sdfFig,[savePath sessionName '_' spikeFields{cellInd(i)} '_ITIsdfs'])
    end
